        ...
        <div class="mid-modal">
          ...
          <div class="column-holder">
            <div class="left-column">
              <h4 v-if="startDateString" class="border-bottom">
                <slot name="start-date-text"></slot>
              </h4>
              <h4 v-if="endDateString" class="border-bottom">
                <slot name="end-date-text"></slot>
              </h4>
              <h4
                v-if="rentalObject && rentalObject.address"
                class="border-bottom"
              >
                <slot name="address">Adress:</slot>
              </h4>
              <h4
                v-if="rentalObject.city && rentalObject"
                class="border-bottom"
              >
                <slot name="city">City:</slot>
              </h4>
              <h4 class="border-bottom"><slot name="beds-text"></slot></h4>
              <h4 class="border-bottom"><slot name="beds-text-two"></slot></h4>
            </div>
            <div class="right-column">
              <p v-if="startDateString" class="border-bottom">
                {{ startDateString }}
              </p>
              <p v-if="endDateString" class="border-bottom">
                {{ endDateString }}
              </p>
              <p v-if="rentalObject.address" class="border-bottom">
